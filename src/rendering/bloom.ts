...
export function drawBloomPass(
  bloomCtx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D,
  cells: CellSnapshot[],
  ...
  for (const cell of cells) {
    ...
    if (tight) {
      const br = sr * 1.5
      const g = (bloomCtx as CanvasRenderingContext2D).createRadialGradient(sx, sy, 0, sx, sy, br)
      ...
      ;(bloomCtx as CanvasRenderingContext2D).fillStyle = g
      bloomCtx.fill()
    } else {
      const br = sr * (4 + ef * 2)
      const g = (bloomCtx as CanvasRenderingContext2D).createRadialGradient(sx, sy, 0, sx, sy, br)
